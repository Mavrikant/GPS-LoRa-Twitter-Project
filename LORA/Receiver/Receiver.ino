...
// paquete transmision
unsigned char txbuf[longitud_areadatos];
// paquete recepcion
...
void loop() {
  ...
  reset_sx1276(); 
  Configuracion_SX1276();  	                                // initializar el modulo RF
  ...
  while(1){
    ...
    if(temporal & 0x40){                                        // esperar la bandera rxdone
      ...
      temporal = SPIleeRegistro(LR_RegFifoRxCurrentaddr);	// read RxCurrentaddr
      SPIescribeRegistro(LR_RegFifoAddrPtr,temporal);		// RxCurrentaddr -> FiFoAddrPtr
      
      longitud_paquete = SPIleeRegistro(LR_RegRxNbBytes);	// leer la longitud del paquete
      SPIleeRafaga(0x00, rxbuf, longitud_paquete);	        // leer desde la fifo
      ...
      //"prueba"
       for (i=0; i < 30; i++){
        ...
        Serial.print("\n");
      if( (rxbuf[9] == ',') && (rxbuf[15] == '.') && (rxbuf[20] == ',') && (rxbuf[27] == '.'))           // verificacion simple del paquete, usar el registro de CRC para mayor robustez
      {
...
//	SPIescribeRegistro(LR_RegFrLsb,0x00);		// frequency：915hz					 
  setFrequency(868000000);          // frequency：868Mhz     
  SPIescribeRegistro(LR_RegPaConfig,0xff); // max output power
  +
...
bool setFrequency(uint32_t freq)
{
 ...
 SPIescribeRegistro(LR_RegFrMsb,reg[0]);
  SPIescribeRegistro(LR_RegFrMid,reg[1]);
  SPIescribeRegistro(LR_RegFrLsb,reg[2]);

  // read if the value has been in register
  if((reg[0]!=SPIleeRegistro(LR_RegFrMsb))||(reg[1]!=SPIleeRegistro(LR_RegFrMid))||(reg[2]!=SPIleeRegistro(LR_RegFrLsb)))
    return false;
}
